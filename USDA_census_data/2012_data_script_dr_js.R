...
# data.folder<-"G:/My Drive/VT/Research/Virginia/USGS_WUDR_AgWithdrawals/USDA_NASS_Data"
# # Note that you'll have to change the path directory 
# # to the location where you've saved the data
# setwd(data.folder) 

# Full dataset for all of US
# data <- read.table(file="2012_cdqt_data.txt", sep="\t", header=TRUE)
...
dim(va.data) 
# write.csv(va.data, "VA_Census_2012_Full.csv")
...
sum(!is.na(irr.land$VALUE)) # 89 counties without NA data
dim(irr.land) #98 counties
length(unique(irr.land$COUNTY_CODE)) #98 unique counties
...
write.csv(farm.size.ops,file="binned_operations.csv")
write.csv(farm.size.size,file="binned_irrigatedarea.csv")
