...
#binned_operations- The no of operations in each bin size for that census year
#binned_irrigated_area - Irrigated area in each bin size for that census year
...
p2<-ggscatter(Non.Reported, x="Total.Irri.Area", y="Pct.under.TH.of.total.Irr.area", 
              add = "reg.line", conf.int = TRUE, 
              cor.coef = TRUE, cor.method = "pearson",
              title = paste0(YEAR, " DEQ facilties and Census Irrigated Operations"),
              xlab = "Total Irrigated Area",
              ylab = "Percentage under threshold")+ scale_y_log10() +scale_x_log10()
p2
...
################### Files Details ########################################################
##########################################################################################
### absent : Some data for 1997 not available for county summaries
### Bin.char: It is the summary of the data for each size bin i.e. counties with D values and without and average irr. area in each size bins
### Binned_irrigated , Binned_operations is the data for particular year the function was run
### County Summary : Is the County data availabe for that particular year
### dat_D_filled: recaled data for missing "D" values
### nass.... : are the dfata for all years from NASS 
### operations list: no of operation in each county by size bins
...
p1<-tm_shape(plotdat)+
  tm_polygons("Irr.Area.Under.TH", title = "Irrigated Area ",
              ...
              id="NAMELSAD",
              legend.hist = TRUE)+
  tm_layout(main.title = paste0(YEAR," Irrigated Area under threshold  (acres)"),
            legend.outside = TRUE,
...
tmap_save(p1, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Irr.Area.Under.TH.png"),  width = 10, height = 5, units = 'in')
tmap_save(p1, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Irr.Area.Under.TH.html"),  width = 10, height = 5, units = 'in')
...
p2<-tm_shape(plotdat)+
  tm_polygons("Pct.under.TH.of.total.Irr.area", title = "Percentage Under threshold ",
              ...
              id="NAMELSAD",
              legend.hist = TRUE)+
  tm_layout(main.title = paste0(YEAR," Percentage of total irrigated area under threshold "),
            legend.outside = TRUE,
            legend.title.size = 1.2,
            legend.text.size = 0.8,
            legend.position = c("left","top"),
...
tmap_save(p2, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Pct.under.TH.of.total.Irr.area.png"),  width = 10, height = 5, units = 'in')
tmap_save(p2, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Pct.under.TH.of.total.Irr.area.html"),  width = 10, height = 5, units = 'in')
...
p3<-tm_shape(plotdat)+
  tm_polygons("Pct.under.TH.of.Irri.area.abv.TH", title = "Percentage Under threshold ",
              ...
              id="NAMELSAD",
              legend.hist = TRUE)+
  tm_layout(main.title = paste0(YEAR," Percentage Irrigated area under threshold of irrigated area above threshold "),
            legend.outside = TRUE,
            legend.title.size = 1.2,
            legend.text.size = 0.8,
            legend.position = c("left","top"),
...
#   tm_add_legend(
#     labels = c('0-42','42-125','125-210','210-380', '380-515', 
#                'All Irrigated area under threshold'))

p3

tmap_save(p3, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Pct.under.TH.of.Irri.area.abv.TH.png"),  width = 10, height = 5, units = 'in')
tmap_save(p3, paste0(WUDR_github,"/plots/Census_Area_under-TH/",YEAR,"_Pct.under.TH.of.total.Irr.area.html"),  width = 10, height = 5, units = 'in')

p4<-tm_shape(plotdat)+
  tm_polygons("AcreageUnderTh", title = "Irrigated Acreage Under Threshold ",
              # breaks = c(0,1,5,10,20),
              n=5,style="jenks",
              id="NAMELSAD",
              legend.hist = TRUE)+
  tm_layout(main.title = paste0(YEAR," Irrigated Acreage Under Threshold  (acres)"),
            legend.outside = TRUE,
            legend.title.size = 1.2,
            legend.text.size = 0.8,
            legend.position = c("left","top"),
...
QS_data(2002)
fn_Area_TH(10, 2002)
# #write.csv(Non.Reported, paste0(WUDR_github, "/csv_files/2002_Under_TH_Summary.csv"))
fn_plot_census(2002)


QS_data(2007)
fn_Area_TH(10,2007)
# #write.csv(Non.Reported,paste0(WUDR_github, "/csv_files/2007_Under_TH_Summary.csv"))
fn_plot_census(2007)

QS_data(2012) 
fn_Area_TH(10, 2012)
# #write.csv(Non.Reported,paste0(WUDR_github, "/csv_files/2012_Under_TH_Summary.csv"))
fn_plot_census(2012)


QS_data(2017)
fn_Area_TH(10)
# #write.csv(Non.Reported,paste0(WUDR_github, "/csv_files/2017_Under_TH_Summary.csv"))
fn_plot_census(2017)
