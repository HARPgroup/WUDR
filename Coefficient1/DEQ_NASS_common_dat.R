...
commondat <- function(YEAR){
##Load Summary to get the data in both datasets
summary_dat<-read.csv(paste0(WUDR_github,"/csv_files/" , YEAR, "_deq_census_dat_avaliability_summary.csv"))
...
#Load DEQ dataset
deq_dat<-read.csv(paste0(WUDR_github,"/csv_files/" , YEAR, "_deq_dat_availability_summary.csv"))
deq_dat <- deq_dat [, -c(1)]
...
#Load Census Data
census_dat<-read.csv(paste0(WUDR_github,"/csv_files/" , YEAR, "_census_dat_availability_summary.csv"))
census_dat <- census_dat[, -c(1)]

#Here NA for acreage is more

#Corelation datset for deq
core_deq<-merge.data.frame(both_datasets_data,deq_dat, by.x = "Countycode", by.y ="Countycode" )

core_deq<-core_deq %>%
  dplyr::select(Countycode,GEOID=GEOID.x,NAMELSAD=NAMELSAD.x,Count,Facility_withdrawal_mg)


core_census<-merge.data.frame(both_datasets_data,census_dat, by.x = "Countycode", by.y ="County_Code" )

core_census<-core_census %>%
  dplyr::select(Countycode=Countycode,GEOID=GEOID.x,NAMELSAD,Irrigated_Acreage,Irrigated_Operations)
...
res <- cor.test(dat$`DEQ No of Facilities`, dat$`Census No of Irrigated Operations`, 
                method = "pearson")
Corelation_DEQ_fac_Census_Operations <<-res #Moderate
...
res <- cor.test(dat$`DEQ reported withdrawals`, dat$`Census Irrigated Acreage`, 
                method = "pearson")
Corelation_DEQ_withdr_Census_Acreage <<-res #Strong
...
dat<-merge.data.frame(core_deq,core_census)
 # write.csv(dat, paste0(WUDR_github,"/csv_files/", YEAR, "Common_counties_dat.csv"))

p<-ggscatter(dat, x="Facility_withdrawal_mg", y="Irrigated_Acreage", 
             add = "reg.line", conf.int = TRUE, 
             cor.coef = TRUE, cor.method = "pearson",
             title = paste0(YEAR, " DEQ withdrawals and Census Irrigated acreage"),
             xlab = "Facility Withdrawal million gallon (DEQ)",
             ylab = "Irrigated Acreage (USDA)")+ scale_y_log10() +scale_x_log10()
p

# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "Acreage_withdarawal_log.png"),p, width = 8, height = 5, units="in")

p2<-ggscatter(dat, x="Count", y="Irrigated_Operations", 
              add = "reg.line", conf.int = TRUE, 
              cor.coef = TRUE, cor.method = "pearson",
              title = paste0(YEAR, " DEQ facilties and Census Irrigated Operations"),
              xlab = "Agriculture facilities (DEQ)",
              ylab = "Irrigated Operations (USDA)")+ scale_y_log10() +scale_x_log10()
p2

# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "Facilties_operations_log.png"),p2, width = 8, height = 5, units="in")
...
# write.csv(rank_dat, paste0(WUDR_github,"/csv_files/", YEAR, "Rand and Difference in Ranks.csv"))


p3<-ggplot(data = rank_dat, aes(x=Acreage_withdarwal_difference))+
  geom_histogram()+
  labs(subtitle = paste0(YEAR," Difference in county ranks for Irrigated acreage in Cenus data and DEQ reported withdrawals"), 
       x= "Difference in rank", y="Count", caption = "(-ve sign of rank indicates higher acreage area rank for a county in census data)")+
  theme_light()


p3
# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "Rank_diff_acreage_withdarwals.png"),p3, width = 8, height = 5, units="in")

p4<-ggplot(data = rank_dat, aes(x=Facilities_Operations_difference))+
  geom_histogram()+
  labs(subtitle = paste0(YEAR, " Difference in county ranks for Operations in Cenus data and Faciltiites in DEQ data"), 
       x= "Difference in rank", y="Count", caption = "(-ve sign of rank indicates higher operations rank for a county in census data)")+
  theme_light()


p4
# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "Rank_diff_opeartions_fac.png"),p4, width = 8, height = 5, units="in")

p5<-ggplot(data = rank_dat, aes(x=Acreage_withdarwal_difference))+
  geom_histogram(aes(y=..density..), colour="black", fill="white")+
  geom_density(alpha=.2, fill="#FF6666")+
  labs(subtitle = paste0(YEAR, " Density plot of difference in ranks for Irrigated acreage in Cenus data and DEQ reported withdrawals"), 
       x= "Difference in rank", y="Density", caption = "(-ve sign of rank indicates higher acreage area rank for a county in census data)")+
  theme_light()
p5 
# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "density_acreage_with.png"),p5, width = 8, height = 5, units="in")

p6<-ggplot(data = rank_dat, aes(x=Facilities_Operations_difference))+
  geom_histogram(aes(y=..density..), colour="black", fill="white")+
  geom_density(alpha=.2, fill="#FF6666")+
  labs(subtitle = paste0(YEAR, " Density plot of difference in ranks for for Operations in Cenus data and Faciltiites in DEQ data"), 
       x= "Difference in ranks", y="Density", caption = "(-ve sign of rank indicates higher operations rank for a county in census data)")+
  theme_light()
p6 

# ggsave(paste0(WUDR_github,"/plots/commondat/", YEAR, "density_opeartions_fac.png"),p6, width = 8, height = 5, units="in")
...
# write.csv(p80_deq_with, paste0(WUDR_github,"/csv_files/", YEAR, "DEQ 80 percent withdarwals.csv"))
##
...
# write.csv(p80_census_acreage, paste0(WUDR_github,"/csv_files/", YEAR, "USDA 80 percent acreage.csv"))
...
# write.csv(bothdatasets, paste0(WUDR_github,"/csv_files/", YEAR, "Common 80 percent acreage and withdrawal.csv"))

DEQ_not_in_USDA<-subset(p80_deq_with, !(GEOID %in% bothdatasets$GEOID))
# write.csv(DEQ_not_in_USDA, paste0(WUDR_github,"/csv_files/", YEAR, "80 perc withdarwal DEQ_not_in_USDA.csv"))
USDA_not_in_DEQ<-subset(p80_census_acreage, !(GEOID %in% bothdatasets$GEOID))
# write.csv(USDA_not_in_DEQ, paste0(WUDR_github,"/csv_files/", YEAR, "80 perc withdarwal USDA_not_in_DEQ.csv"))
}
