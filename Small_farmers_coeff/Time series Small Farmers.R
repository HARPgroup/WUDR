...
state_summary_Mean_Coeff_Max_UnderTH <- Compare_Mean_Coeff_Max_UnderTH %>% 
  group_by(YEAR) %>% 
  summarise(` (1) Unreported Mean Coeff` = sum(`(b) Mean Coeff`),
            `(2) Unreported Max Under Th`  = sum(`(a) Max Under Th`))

state_summary_Mean_Coeff_Max_UnderTH <- left_join(state_summary_Mean_Coeff_Max_UnderTH, DEQ_Irr, by = "YEAR")

plot_dat <- pivot_longer(state_summary_Mean_Coeff_Max_UnderTH, cols = c(2:4), names_to = "Type" ,values_to = "Unreported Wth")

p1 <- ggplot(plot_dat, aes(x=YEAR, y=`Unreported Wth`, group = Type ))+
  geom_line(aes(color=Type))+
  labs(title= "Small Farmer Unreported Withdrawals" ,
...
ggsave(paste0(WUDR_github,"/plots/Coefficient1/timeseries/Small Farmers/","State_Summary_MeanCoeff_MaxUnderTh.png"), plot = p1, width = 9.5, height = 6, units = "in")

################################################

state_summary_median <- Compare_Median_Coeff_UnderTH %>% 
  group_by(YEAR) %>% 
  summarise(`(1) Unreported Median Coeff` = sum(`(b) Median Coeff`),
            `(2) Unreported Median Under Th`  = sum(`(a) Median Under Th`))

state_summary_median <- left_join(state_summary_median, DEQ_Irr, by = "YEAR")

plot_dat <- pivot_longer(state_summary_median, cols = c(2:4), names_to = "Type" ,values_to = "Unreported Wth")
...
ggsave(paste0(WUDR_github,"/plots/Coefficient1/timeseries/Small Farmers/","State_Summary_MedianCoeff_MedianUnderTh.png"), plot = p2, width = 9.5, height = 6, units = "in")
